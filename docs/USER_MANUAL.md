...
### Quant Factor (퀀트 팩터) — 현재 비활성

여러 지표(가치, 품질, 모멘텀)를 종합 점수로 매겨 상위 종목에 투자합니다.
현재 비활성화 상태이며, 전략 설정 탭에서 활성화할 수 있습니다.
...
알려진 이슈입니다. 현재 수정 작업이 진행 중입니다 (docs/simulation_issues.md 참조).
백테스트 기능은 정상 작동하므로, 전략 검증은 백테스트 탭을 이용해주세요.
...
현재 버전에서 아직 완벽하지 않은 부분입니다.

- **모의거래 수량/가격 표시**: 시그널 프리뷰에서 수량 0주, 가격 ₩0으로 표시되는 문제 (수정 예정)
- **포지션 현재가 갱신**: 매수 후 현재가가 업데이트되지 않아 수익률이 0%로 표시됨 (수정 예정)
- **벤치마크 포트폴리오 라인**: 벤치마크 차트에 내 포트폴리오 수익률 라인이 아직 미연동
- **휴일 처리**: 공휴일에 데이터 수집 시 빈 데이터가 반환될 수 있음
- **캐시 지연**: 실행 후 결과 반영까지 최대 5분 소요

자세한 수정 계획은 `docs/simulation_issues.md`에 정리되어 있습니다.
